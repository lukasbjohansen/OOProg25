...
public abstract class DBMethodsBase<T> where T : IHarId
{
    ...
    /// og returner det tilsvarende objekt.
	...
	/// Det valgte Id for objektet returneres.
	/// </summary>
	...
	/// <summary>
	/// Finder det højeste brugte Id, givet en liste af objekter der alle har et Id.
	/// </summary>
	protected int FindMaxId()
	{
		int maxId = ReadAllFromDB().Select(dr => dr.Id).DefaultIfEmpty(0).Max();
		return maxId;
	}
}
